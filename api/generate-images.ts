...
Return your response as a single, valid JSON array. Do not include any text or markdown formatting outside of the JSON structure. Each object in the array must have two keys: "title" (string) and "description" (string).`;
      ...
      const imagePromises = textData.map((concept: any) => {
...
- **VIEW ANGLE:** Generate a photorealistic **Front Perspective View** of the stand, as seen from the main aisle.`;
      ...
      const concepts = imageResponses.map((res, index) => {
          ...
          return { 
              ...
              image: inlineData ? `data:${inlineData.mimeType};base64,${inlineData.data}` : ''
          };
      });
...
- **REQUIRED VIEW:** The render must be a main **Perspective View** showing the overall look and feel of the space.`;
