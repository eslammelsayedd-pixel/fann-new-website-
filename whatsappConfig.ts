...
export const MESSAGES = {
  ...
  portfolio: "Hi FANN! I saw your stand in the portfolio. Can you quote me for a similar 50sqm stand at Arab Health 2026?",
...
export const getPageContext = (pathname: string) => {
  if (pathname === '/' || pathname === '') return { type: 'homepage', message: MESSAGES.homepage, source: 'homepage' };
  if (pathname.includes('portfolio')) return { type: 'portfolio', message: MESSAGES.portfolio, source: 'portfolio' };
  if (pathname.includes('services')) return { type: 'services', message: MESSAGES.services, source: 'services' };
  if (pathname.includes('contact')) return { type: 'contact', message: MESSAGES.contact, source: 'contact' };
