...
import { Layers, Lightbulb, Wrench, Cpu, Loader2, ServerCrash, ArrowLeft } from 'lucide-react';
...
const DesignResultPage: React.FC = () => {
    ...
    useEffect(() => {
        if (!formData) {
            // Redirect if user lands here directly without form data
                        ...
                        {design && !isLoading && (
                            <motion.div key="result" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                                ...
                                <div className="grid md:grid-cols-2 lg:grid-cols-2 gap-8">
                                    <InfoCard icon={Layers} title="Key Materials" items={design.designConcept.materials} />
                                    <InfoCard icon={Lightbulb} title="Lighting Strategy" items={design.designConcept.lighting} />
                                    <InfoCard icon={Wrench} title="Stand Features" items={formData.features} />
                                    <InfoCard icon={Cpu} title="Technology Integration" items={design.designConcept.technologyFeatures} />
                                </div>
