...
const StudioCard: React.FC<StudioCardProps> = ({ studio, index }) => (
    ...
    >
        <Link 
            ...
            className={`
                h-full flex flex-col justify-between p-8 bg-white dark:bg-fann-teal-dark rounded-lg 
                border-2 border-fann-teal/10 dark:border-fann-border group transition-all duration-300
                ${studio.isComingSoon 
                    ? 'cursor-not-allowed opacity-60' 
                    : 'hover:border-fann-accent-teal dark:hover:border-fann-accent-peach hover:bg-fann-accent-teal/5 dark:hover:bg-fann-accent-peach/5'
                }
            `}
        >
            <div>
                <div className="flex items-center justify-between">
                    <studio.icon className="w-12 h-12 text-fann-accent-teal dark:text-fann-accent-peach" />
                    {studio.isComingSoon && <span className="text-xs bg-fann-light-gray text-fann-teal font-bold uppercase tracking-wider px-2 py-0.5 rounded-full">Soon</span>}
                </div>
                <h3 className="text-3xl font-serif font-bold text-fann-teal dark:text-fann-peach mt-6">{studio.name}</h3>
                <p className="text-fann-teal/80 dark:text-fann-light-gray mt-3 text-base">{studio.description}</p>
            </div>
            {!studio.isComingSoon && (
                <span className="font-semibold text-fann-accent-teal dark:text-fann-accent-peach mt-8 inline-block group-hover:underline">
            ...
            <div className="min-h-screen bg-fann-peach dark:bg-fann-teal pt-32 pb-20 text-fann-teal dark:text-fann-peach">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div className="text-center mb-12">
                        <h1 className="text-5xl md:text-6xl font-serif font-bold text-fann-accent-teal dark:text-fann-accent-peach mb-4">FANN Studio</h1>
