...
type PageState = 'initial' | 'loading' | 'results';
...
const ROICalculatorPage: React.FC = () => {
    ...
    const runCalculation = useCallback(async (data: FormData, isInitialAnalysis: boolean) => {
        isInitialAnalysis ? setPageState('loading') : setIsRecalculating(true);
        setError(null);
        setIsDownloaded(false);
        try {
            const payload = isInitialAnalysis 
            ...
            if (isInitialAnalysis && resultData.suggestions) {
            ...
            setError(e.message);
            setResults(null);
            if (isInitialAnalysis) setPageState('initial');
        } finally {
    ...
    const handleInitialSubmit = (e: React.FormEvent) => {
        ...
        if (!formData.company_name || !formData.event_name) {
            setError("Please provide both company and event names.");
            return;
        }
    ...
    const handleFormChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        const newFormData = { ...formData, [name]: name === 'event_name' || name === 'company_name' ? value : Number(value) };
        setFormData(newFormData);
        // Debounced recalculation
        const handler = setTimeout(() => {
            runCalculation(newFormData, false);
        }, 800);
    ...
    const renderInitialState = () => (
        <motion.div key="initial" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}}>
            <form onSubmit={handleInitialSubmit} className="max-w-2xl mx-auto bg-fann-charcoal-light p-8 rounded-lg space-y-6">
                <input type="text" name="company_name" placeholder="Your Company Name" value={formData.company_name} onChange={handleFormChange} required className="w-full bg-fann-charcoal border border-fann-border rounded-md px-4 py-3 text-lg" />
                 <select name="event_name" value={formData.event_name} onChange={handleFormChange} className="w-full bg-fann-charcoal border border-fann-border rounded-md px-4 py-3 text-lg">
                ...
                <button type="submit" className="w-full bg-fann-gold text-fann-charcoal font-bold py-3 rounded-full flex items-center justify-center gap-2 text-lg">
    ...
    const renderLoadingState = () => (
         <motion.div key="loading" initial={{opacity: 0}} animate={{opacity: 1}} exit={{opacity: 0}} className="text-center">
            ...
            <h2 className="text-3xl font-serif text-white mt-6">Personalizing Your Forecast...</h2>
            <p className="text-fann-light-gray mt-2 max-w-md mx-auto">Analyzing {formData.company_name}'s profile and the specifics of {formData.event_name}. This may take a moment.</p>
        </motion.div>
    );
    ...
    const renderResultsState = () => (
        <motion.div key="results" initial={{opacity: 0}} animate={{opacity: 1}}>
            <div className="text-center mb-4">
                <h2 className="text-3xl font-serif">Personalized Projections for <span className="text-fann-gold">{formData.company_name}</span> at <span className="text-fann-gold">{formData.event_name}</span></h2>
            </div>
            <div className="grid lg:grid-cols-2 gap-8 max-w-7xl mx-auto">
                ...
                <div className="bg-fann-charcoal-light p-6 rounded-lg space-y-6 self-start">
                    ...
                    <p className="text-xs text-fann-light-gray bg-fann-charcoal p-2 rounded-md flex gap-2 items-center"><Sparkles size={16} className="text-fann-gold flex-shrink-0"/> We've pre-filled these fields based on our analysis. Please review and adjust for accuracy.</p>
                     <div>
                        ...
                        <input type="range" id="booth_size_sqm" name="booth_size_sqm" min="9" max="500" step="1" value={formData.booth_size_sqm} onChange={handleFormChange} className="w-full h-2 bg-fann-charcoal-light rounded-lg appearance-none cursor-pointer accent-fann-gold" />
                     </div>
                      <div>
                        <label htmlFor="stand_cost" className="block text-sm font-medium text-fann-light-gray mb-2">Stand Design & Build Cost (AED)</label>
                        <input type="number" step="1000" id="stand_cost" name="stand_cost" value={formData.stand_cost} onChange={handleFormChange} className="w-full bg-fann-charcoal border border-fann-border rounded-md px-3 py-2" />
                    </div>
                    <div className="grid md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="number" step="1000" id="average_deal_size" name="average_deal_size" value={formData.average_deal_size} onChange={handleFormChange} className="w-full bg-fann-charcoal border border-fann-border rounded-md px-3 py-2" />
                        </div>
                         <div>
                            ...
                            <input type="range" id="close_rate" name="close_rate" min="1" max="80" step="1" value={formData.close_rate} onChange={handleFormChange} className="w-full h-2 bg-fann-charcoal-light rounded-lg appearance-none cursor-pointer accent-fann-gold" />
                        </div>
                    </div>
                </div>
                     ...
                     <AnimatePresence>
                        {isRecalculating && (
                            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 bg-fann-charcoal-light/80 backdrop-blur-sm flex flex-col justify-center items-center z-10 rounded-lg">
                                ...
                                <p className="mt-4 font-semibold">Recalculating...</p>
                            </motion.div>
                        )}
                    </AnimatePresence>
                            ...
                            <motion.div initial={{opacity: 0}} animate={{opacity: 1}} className="space-y-6">
                                ...
                                <div>
                                    ...
                                    <div className="grid md:grid-cols-3 gap-4">
                                        <div className="bg-fann-charcoal p-4 rounded-lg flex items-center gap-4"><Users className="w-8 h-8 text-fann-gold flex-shrink-0" /><div><p className="text-sm text-fann-light-gray">Qualified Leads</p><p className="text-xl font-bold text-white">{`${results.visitor_projections.qualified_leads_min} - ${results.visitor_projections.qualified_leads_max}`}</p></div></div>
                                        <div className="bg-fann-charcoal p-4 rounded-lg flex items-center gap-4"><TrendingUp className="w-8 h-8 text-fann-gold flex-shrink-0" /><div><p className="text-sm text-fann-light-gray">Expected Revenue</p><p className="text-xl font-bold text-white">{`${formatCurrency(results.financial_projections.expected_revenue_min)}`}</p></div></div>
                                        <div className="bg-fann-charcoal p-4 rounded-lg flex items-center gap-4"><Target className="w-8 h-8 text-fann-gold flex-shrink-0" /><div><p className="text-sm text-fann-light-gray">Deals to Break-Even</p><p className="text-xl font-bold text-white">{`${results.roi_metrics.break_even_deals}`}</p></div></div>
                                    </div>
                                </div>
                                
                                <AnimatePresence mode="wait">
                                {isDownloaded ? (
                                    <motion.div key="thank-you" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-fann-charcoal p-6 rounded-lg text-center border-2 border-fann-teal">
                                    ...
                                    <motion.form key="lead-form" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} onSubmit={handleDownloadReport} className="bg-fann-charcoal p-6 rounded-lg border border-fann-border">
