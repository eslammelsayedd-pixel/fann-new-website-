...
import { FileText, Crown, Palette, Sparkles, SlidersHorizontal, Users } from 'lucide-react';
                    ...
                    <form onSubmit={handleSubmit} className="bg-white dark:bg-fann-accent-teal p-8 rounded-lg space-y-8">
                        <FormSection title="The Brief" icon={FileText}>
                            <div className="grid md:grid-cols-2 gap-6">
                                ...
                                <input type="text" name="eventName" placeholder="Event Name (e.g., Annual Gala)" value={formData.eventName} onChange={handleInputChange} required className="w-full bg-fann-peach/50 dark:bg-fann-teal border border-fann-teal/20 dark:border-fann-border rounded-md px-4 py-3" />
                            </div>
                        </FormSection>

                        <FormSection title="Event Scale" icon={Users}>
                             <div>
                                <label className="block text-sm font-medium mb-2">Number of Guests: <span className="font-bold text-fann-accent-teal dark:text-fann-gold">{formData.guestCount}</span></label>
                            ...
                            <div className="mt-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                    {eventTypes.map(type => (
                                        <button key={type} type="button" onClick={() => handleOptionClick('eventType', type)} className={`px-3 py-2 text-sm font-semibold rounded-md transition-all ${formData.eventType === type ? 'bg-fann-gold text-fann-charcoal' : 'bg-fann-peach/50 dark:bg-fann-teal-dark hover:bg-fann-peach'}`}>{type}</button>
                                    ))}
                                </div>
                            </div>
                        </FormSection>
