...
import { FileText, Crown, Palette, Sparkles, SlidersHorizontal, Users, Check } from 'lucide-react';
    ...
    return (
        <AnimatedPage>
            <SEO
                ...
                description="Create a stunning mood board and concept visual for your next corporate event. From galas to launches, bring your theme to life with FANN."
            />
            <div className="min-h-screen bg-fann-peach dark:bg-fann-teal pt-32 pb-20 text-fann-teal dark:text-fann-peach">
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-4xl">
                    <div className="text-center mb-12">
                         <Crown className="mx-auto h-16 w-16 text-fann-accent-teal dark:text-fann-gold" />
                        <h1 className="text-5xl font-serif font-bold text-fann-accent-teal dark:text-fann-gold mt-4 mb-4">Event Studio</h1>
                        <p className="text-xl text-fann-teal/90 dark:text-fann-peach/90">
                           Complete the brief below to generate a bespoke concept for your event.
                        </p>
                    </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-0">
                            ...
                            <FormSection step={1} title="The Brief" icon={FileText} description="Tell us about your company and event.">
                                <div className="grid md:grid-cols-2 gap-6">
                                    ...
                                    <input type="text" name="eventName" placeholder="Event Name (e.g., Annual Gala)" value={formData.eventName} onChange={handleInputChange} required className="w-full bg-fann-peach/50 dark:bg-fann-teal border border-fann-teal/20 dark:border-fann-border rounded-md px-4 py-3 focus:outline-none focus:ring-2 focus:ring-fann-accent-teal dark:focus:ring-fann-gold" />
                                </div>
                            </FormSection>
                            ...
                            <FormSection step={2} title="Event Scale" icon={Users} description="Specify the size and type of your event.">
                                <div>
                                    <div className="flex justify-between items-center mb-2">
                                        ...
                                        <span className="font-bold text-lg text-fann-accent-teal dark:text-fann-gold bg-fann-peach/50 dark:bg-fann-teal px-2 py-0.5 rounded-md">{formData.guestCount}</span>
                                    </div>
                                    <input type="range" name="guestCount" min="50" max="2000" step="50" value={formData.guestCount} onChange={handleSliderChange} className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-fann-teal/20 dark:bg-fann-border accent-fann-accent-teal dark:accent-fann-gold" />
                                    <div className="flex justify-between text-xs text-fann-light-gray mt-1"><span>50</span><span>2000</span></div>
                                </div>
                                <div className="mt-8">
                                    <label className="block text-sm font-medium mb-3">Event Type</label>
                                    <div className="grid grid-cols-2 lg:grid-cols-4 gap-2">
                                        {eventTypes.map(type => (
                                            <button key={type} type="button" onClick={() => handleOptionClick('eventType', type)} className={`px-3 py-2 text-sm font-semibold rounded-md transition-all flex items-center justify-center ${formData.eventType === type ? 'bg-fann-gold text-fann-charcoal' : 'bg-fann-peach/50 dark:bg-fann-teal-dark hover:bg-fann-peach'}`}>
                                                {formData.eventType === type && <Check size={16} className="mr-2 flex-shrink-0" />}
                                                {type}
                                            </button>
                                        ))}
                                    </div>
