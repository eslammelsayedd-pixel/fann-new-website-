...
const InteriorStudioPage: React.FC = () => {
    ...
    const generateConcepts = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch('/api/generate-images', {
                ...
                body: JSON.stringify({ promptDataInterior, formData })
            });
    ...
    // --- Main Render Logic ---
    if (isLoading) return <div className="min-h-screen flex flex-col justify-center items-center text-center p-4"><Loader2 className="w-16 h-16 text-fann-gold animate-spin" /><h2 className="text-3xl font-serif text-white mt-6">Crafting Your Designs...</h2><p className="text-fann-light-gray mt-2 max-w-sm">Our design engine is interpreting your brief and generating bespoke concepts. This may take up to a minute.</p></div>;
                            ...
                            <div className="bg-fann-charcoal-light p-6 sm:p-8 rounded-lg min-h-[500px]">
                                <form onSubmit={generateConcepts} noValidate>
                                    ...
                                    <div className="mt-8 pt-6 border-t border-fann-border">
                                        {error && <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-red-900/50 border border-red-500 text-red-300 px-4 py-3 rounded-lg text-sm flex items-start gap-3 mb-4"><AlertCircle className="w-5 h-5 flex-shrink-0 mt-0.5" /><span className="whitespace-pre-wrap">{error}</span> {error.includes("limit") && ( <a  href={`https://wa.me/971505667502?text=${encodeURIComponent("Hello FANN, I've reached my design limit on the FANN Studio and would like to discuss my project further.")}`} target="_blank" rel="noopener noreferrer" className="bg-green-500 text-white font-bold px-4 py-2 rounded-md ml-auto" > Chat on WhatsApp </a> )}</motion.div>}
