...
import { FileText, PenTool, Palette, Sparkles, SlidersHorizontal, Square, Check, ArrowLeft, ArrowRight } from 'lucide-react';
        ...
        switch (currentStep) {
                ...
                return (
                    <div className="space-y-6">
                         <div className="text-center mb-6">
                            <h2 className="text-2xl font-serif font-bold text-fann-peach">Project Brief</h2>
                            <p className="text-fann-light-gray">Let's start with the basics.</p>
                        </div>
                        <div className="grid md:grid-cols-2 gap-6">
                             <div className="space-y-2">
                                <label className="text-sm font-medium text-fann-gold ml-1">Project Name</label>
                                <div className="relative">
                                    <FileText className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={20} />
                                    <input type="text" name="projectName" placeholder="e.g. Downtown HQ" value={formData.projectName} onChange={handleInputChange} required className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 focus:outline-none focus:border-fann-gold focus:ring-1 focus:ring-fann-gold transition-all text-white placeholder-gray-500" />
                                </div>
                            </div>
                             <div className="space-y-2">
                                <label className="text-sm font-medium text-fann-gold ml-1">Client/Company</label>
                                <div className="relative">
                                    <FileText className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-500" size={20} />
                                    <input type="text" name="clientName" placeholder="Your Name" value={formData.clientName} onChange={handleInputChange} required className="w-full bg-white/5 border border-white/10 rounded-xl pl-12 pr-4 py-4 focus:outline-none focus:border-fann-gold focus:ring-1 focus:ring-fann-gold transition-all text-white placeholder-gray-500" />
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 2:
                return (
                    <div className="space-y-8">
                         <div className="text-center mb-6">
                            <h2 className="text-2xl font-serif font-bold text-fann-peach">Space Definition</h2>
                            <p className="text-fann-light-gray">Area size and usage type.</p>
                        </div>

                        <div className="bg-white/5 p-6 rounded-2xl border border-white/10">
                            <div className="flex justify-between items-center mb-4">
                                <span className="font-bold text-lg text-white flex items-center gap-2"><Square size={20}/> Total Area</span>
                                <span className="text-fann-gold font-mono bg-fann-gold/10 px-3 py-1 rounded-lg">
                                    {formData.spaceArea} sqm
                                </span>
                            </div>
                            <input type="range" name="spaceArea" min="50" max="3000" step="10" value={formData.spaceArea} onChange={handleSliderChange} className="w-full h-2 rounded-lg appearance-none cursor-pointer bg-gray-700 accent-fann-gold" />
                             <div className="flex justify-between text-xs text-gray-400 mt-2"><span>50 sqm</span><span>3000 sqm</span></div>
                        </div>

                         <div>
                            <label className="block text-sm font-medium text-fann-gold mb-3 ml-1">Space Type</label>
            ...
            case 3:
                 return (
                    <div className="space-y-6">
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-serif font-bold text-fann-peach">Design Style</h2>
                            <p className="text-fann-light-gray">Select the aesthetic direction.</p>
                        </div>
            ...
            case 4:
                return (
                    <div className="space-y-6">
                        <div className="text-center mb-6">
                            <h2 className="text-2xl font-serif font-bold text-fann-peach">Functionality</h2>
                            <p className="text-fann-light-gray">Key features to include.</p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                         ...
                         <div className="bg-white/5 p-4 rounded-xl border border-white/10 mt-6">
                            ...
                            <ul className="text-sm text-gray-400 space-y-1">
                                <li>• {formData.projectName} ({formData.clientName})</li>
                                <li>• ~{formData.spaceArea} sqm {formData.spaceType}</li>
                ...
                <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
                    <div className="text-center mb-10">
                         <div className="inline-flex items-center justify-center p-3 bg-fann-gold/10 rounded-full mb-4">
                            <PenTool className="h-8 w-8 text-fann-gold" />
                        </div>
                        <h1 className="text-4xl md:text-5xl font-serif font-bold text-white mb-2">Interior Studio</h1>
                        <p className="text-lg text-gray-400">Design your perfect space in 4 simple steps.</p>
                    </div>
